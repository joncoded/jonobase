...
import { timezone, colors } from '@/lib/app.config'
...
const OpusLine = ({opus : { _type, kind, slug, image, emoji, title, subtitle, link, date, showDate}, showType = false, showKind = false}: ListOpusProps) => {  
  ...
  const Card = ({children} : UtilDOMChildrenProps) => {
    return (
      <article className={`card-unit w-full font-sans`}>
  ...
  const CardMeta = () => {
    ...
    return (
      <div className={`card-meta text-center text-sm`}>
        ...
        {showType === true && <p className={`!my-0`} aria-hidden="true"><Link className={linkColors} href={`/${_type}s`}>({_type})</Link></p>}
        {showKind === true && <p className={`!my-0`} aria-hidden="true">[<Link className={linkColors} href={`/${_type}s/${kind}`}>{kind}</Link>]</p>}
      </div>
    )
  }
  ...
  const CardExternalLink = () => {
    return (
      <div className={`card-link my-5 ${!link && `hidden`}`}>
        {link && 
          <Link 
            className={`card-link-link px-5 py-2 bg-white text-black text-lg 
