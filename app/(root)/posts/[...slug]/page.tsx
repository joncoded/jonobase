      ...
      <Sect className={`post-turn !bg-gradient-to-r from-white dark:from-black to-zinc-50 dark:to-gray-900 p-5 text-lg md:text-xl font-sans`}>
        ...
        { newerPost && 
          <div className={`text-center md:text-left`}>
            {text['post newer']} <br /> 
            <Link className="text-sky-600 dark:text-lime-300 hover:text-black dark:hover:text-white hover:underline" href={newerPost.slug}>{newerPost.title}</Link>
          </div>
        }
        { olderPost && 
          <div className={`text-center md:text-right`}>
            {text['post older']} <br /> 
            <Link className="text-sky-600 dark:text-lime-300 hover:text-black dark:hover:text-white hover:underline" href={olderPost.slug}>{olderPost.title}</Link>
          </div>
        }
        </div>
      </Sect>
